...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"):
        #This closes app, prevents memory leaks
        self.master.destroy()
        os._exit(0)
...
def addToList(self):
    ...
    var_fname = var_fname.title() #ensures that the first character in each word is capitalized
...
def onDelete(self):
    ...
    with conn:
        ...
        #the database... cannot delete last record or we will get a error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
